...
class MapAppppss(tk.Tk):
    ...
    def print_linked_list(self):
        if not self.nodes:
            ...
            return
        print("\n=== All Node Connections (by lines) ===")
        for line in self.lines:
            # For straight lines, use node_ids if available, else infer from points
            if 'node_ids' in line:
                node_ids = line['node_ids']
                if len(node_ids) == 2:
                    print(f"Node {node_ids[0]} -> Node {node_ids[1]} (type: {line['type']})")
            elif line['type'] == 'straight' and len(line['points']) == 4:
                # Try to infer node indices from coordinates
                x1, y1, x2, y2 = line['points']
                n1 = min(self.nodes, key=lambda n: (n.x-x1)**2 + (n.y-y1)**2)
                n2 = min(self.nodes, key=lambda n: (n.x-x2)**2 + (n.y-y2)**2)
                print(f"Node {n1.index} -> Node {n2.index} (type: straight)")
            elif line['type'] == 'curve' and 'points' in line and len(line['points']) >= 4:
                # For curves, print first and last node
                x1, y1 = line['points'][0], line['points'][1]
                x2, y2 = line['points'][-2], line['points'][-1]
                n1 = min(self.nodes, key=lambda n: (n.x-x1)**2 + (n.y-y1)**2)
                n2 = min(self.nodes, key=lambda n: (n.x-x2)**2 + (n.y-y2)**2)
                print(f"Node {n1.index} -> Node {n2.index} (type: curve)")
        print("="*35)
